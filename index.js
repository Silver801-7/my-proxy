...
function isForcedCompression(req) {
  const rawQuality = getRequestedQuality(req);
  return Number.parseInt(rawQuality, 10) === 95;
}
...
app.get('/', async (req, res) => {
  ...
  if (!imageUrl) {
    return res.status(200).send('v3.1-Redirect-On-403');
  }
  ...
  try {
    ...
    const dynamicThresholdKB = 100 + (quality - 10) * 5;
    ...
    const forceCompression = isForcedCompression(req);
    const shouldResize = fileSizeInKB > dynamicThresholdKB;

    /*
     * قيمة 95 وضع خاص:
     * - نضغط كل الصور بجودة فعلية 20.
     * - نتجاهل حد الحجم وtargetWidth.
     * - لا نستخدم grayscale() حتى تبقى الصور الملونة ملونة.
     * - لا نستخدم isGrayscale حتى لا تمسح ألوان الصورة.
     */
    if (forceCompression) {
      const forcedBuffer = await sharp(responseBuffer, {
        failOn: 'none',
      ...
      res.set({
        'Content-Type': 'image/jpeg',
        'Content-Length': forcedBuffer.length,
        'Cache-Control': 'public, max-age=31536000, immutable',
        'X-Proxy-Version': '2.9-MultiFallback',
        'X-Proxy-Debug': `FORCED_COMPRESSED|requestedQuality=95|quality=${quality}|sourceBytes=${responseBuffer.length}|outputBytes=${forcedBuffer.length}|format=${metadata.format}`,
        'X-Source-Format': metadata.format,
      });

      return res.status(200).send(forcedBuffer);
    }
    ...
    if (req.query.debug === '1' || req.query.debug === 'true') {
      return res.status(502).json({
        proxyVersion: '3.1-Redirect-On-403',
        requestedUrl: imageUrl,
    ...
    /*
     * إذا رفض المصدر طلب الـ server بـ 403 بينما يفتح الرابط مباشرةً في
     * المتصفح، نعيد توجيه العميل إلى الرابط الأصلي بدلاً من إرسال صورة
     * fallback. وبذلك يستطيع المتصفح جلب الصورة مباشرةً من المصدر.
     */
    if (errorStatus === 403 && parsedUrl) {
      res.set({
        Location: imageUrl,
        'Cache-Control': 'no-store',
        'X-Proxy-Version': '3.1-Redirect-On-403',
        'X-Proxy-Status': 'Upstream-403-Redirect',
      });
